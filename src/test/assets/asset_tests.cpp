...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of YERBASCOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("YERB", type));
        BOOST_CHECK(!IsAssetNameValid("YERBAS", type));
        BOOST_CHECK(!IsAssetNameValid("YERBASCOIN", type));

        //- Versions of YERBASCOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("YERBAS.COIN", type));
        BOOST_CHECK(IsAssetNameValid("YERBAS_COIN", type));
        BOOST_CHECK(IsAssetNameValid("YERBSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERYERB", type));
        BOOST_CHECK(IsAssetNameValid("YERBASSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEYERBAS", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_YERBASS", type));
        BOOST_CHECK(IsAssetNameValid("SEYERBOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        CAssetTransfer asset("YERBAS", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("YERBAS", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
